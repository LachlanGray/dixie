...
class FileScout(lloam.Agent):
    ...
    def start(self):
        ...
        todo = list(self.completed_dirs.values())
        self.log(f"{len(todo)} tasks deployed", level="finished")
    ...
    def explore_dir(self, dir_path):
        ...
        self.dirs[dir_path] = (subdirs, files)


        for subdir in subdirs.choices.stream():
            ...
            if self.explorer.is_dir(subdir_path):
                ...
                self.explore_dir(subdir_path)
            else:
                self.log(f"nonexistent directory {subdir_path}", level="warning")


        for file in files.choices.stream():
            ...
            self.files[file_path] = self.describe_file(dir_path, file)
    ...
    @lloam.prompt()
    def describe_file(self, directory, file):
        """
        {self.initial_query}
        ...
        ```
        {self.explorer.cat(directory + '/' + file)}
        ```
